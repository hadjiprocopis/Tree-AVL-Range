...
sub	make_node_and_add {
	...
	if( ! defined($bi) ){ carp 'Tree::AVL::Range::make_node_and_add()'." : call to $self->{'new-node-sub'} has failed for params ".Data::Dump::dump($params)."\n"; return undef }
...
#	if( defined($m=$self->contains($abin->right_boundary())) ){ carp 'Tree::AVL::Range::add_node()'." : bin to insert ".$abin." overlaps with already existing bin: ".$m; return 0 }
	...
	try {
		$self->insert($abin);
	} catch { carp 'Tree::AVL::Range::add_node()'." : call to insert() has failed for bin: ".$abin; return 0 };
