...
ok(defined($B1), "Tree::AVL::Range::Node1D->new() OK");
...
ok(defined($B2), "Tree::AVL::Range::Node1D->new() OK");
...
ok(defined($B3), "Tree::AVL::Range::Node1D->new() OK");
...
ok(defined($B4), "Tree::AVL::Range::Node1D->new() OK");
...
ok(defined($B1), "Tree::AVL::Range::Node1D->new() OK") or BAIL_OUT("call to ".'Tree::AVL::Range::Node1D->new()'." has failed, B1.");
foreach(
	[-1,-1],
	[0, 0],
	[0.1, 0],
	...
	is($B1->compare_scalar($_->[0]), $_->[1], "compare_scalar() : compares with scalar ".$_->[0].', expects : '.$_->[1]);
	is($B1->contains($_->[0]), $_->[1]==0, "contains() : contains scalar ".$_->[0].', expects : '.$_->[1]);
}
...
$B2 = $B1->clone();
ok(defined($B2), "clone() returned ok") or BAIL_OUT("call to ".'Tree::AVL::Range::Node1D::clone()'." has failed, B2 (clone)\n");
